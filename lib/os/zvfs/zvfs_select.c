...
{
	struct zvfs_pollfd pfds[CONFIG_NET_SOCKETS_POLL_MAX];
			      ...
			      const void *ZRESTRICT sigmask)
{
	...
	if (timeout) {
		timeval = k_usermode_alloc_from_copy((void *)timeout, sizeof(*timeval));
		if (!timeval) {
			errno = ENOMEM;
			goto out;
		}
	...
	ret = z_impl_zvfs_select(nfds, readfds_copy, writefds_copy, exceptfds_copy, timeval,
				 sigmask);
