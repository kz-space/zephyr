 ...
 *   addresses. We instantiate a struct mm_vm_page_frame to store metadata for
 *   every page frame.
 *
...
struct mm_vm_page_frame mm_vm_page_frames[MM_VM_NUM_PAGE_FRAMES];

#if __ASSERT_ON
/* Indicator that mm_vm_page_frames has been initialized, many of these APIs do
...
/* LCOV_EXCL_START */
static void page_frame_dump(struct mm_vm_page_frame *pf)
{
	if (mm_vm_page_frame_is_free(pf)) {
		COLOR(GREY);
		printk("-");
	} else if (mm_vm_page_frame_is_reserved(pf)) {
		COLOR(CYAN);
		printk("R");
	} else if (mm_vm_page_frame_is_busy(pf)) {
		COLOR(MAGENTA);
		printk("B");
	} else if (mm_vm_page_frame_is_pinned(pf)) {
		COLOR(YELLOW);
		printk("P");
	} else if (mm_vm_page_frame_is_available(pf)) {
		COLOR(GREY);
		printk(".");
	} else if (mm_vm_page_frame_is_mapped(pf)) {
		COLOR(DEFAULT);
		printk("M");
	} else {
		COLOR(RED);
...
void mm_vm_page_frames_dump(void)
{
	...
	printk("Physical memory from 0x%lx to 0x%lx\n",
	       MM_PHYS_RAM_START, MM_PHYS_RAM_END);

	for (int i = 0; i < MM_VM_NUM_PAGE_FRAMES; i++) {
		struct mm_vm_page_frame *pf = &mm_vm_page_frames[i];
 ...
 * virtual memory in the region MM_KERNEL_VIRT_START to
 * MM_KERNEL_VIRT_END. Unused virtual memory past this, up to the limit
 ...
 * +--------------+ <- MM_KERNEL_VIRT_START (often == MM_VIRT_RAM_START)
 * | Mapping for  |
 * | main kernel  |
 * | image        |
 * |		  |
...
#define Z_VIRT_REGION_START_ADDR	MM_VM_FREE_START
#define Z_VIRT_REGION_END_ADDR		(MM_VIRT_RAM_END - MM_VM_RESERVED)

static inline uintptr_t virt_from_bitmap_offset(size_t offset, size_t size)
{
	return POINTER_TO_UINT(MM_VIRT_RAM_END)
...
static void virt_region_init(void)
{
	 ...
	 * k_mem_map() and mm_memmap_phys_bare_map(). Mark them as
	 * already allocated so they will never be used.
	 */

	if (MM_VM_RESERVED > 0) {
		/* Mark reserved region at end of virtual address space */
		num_bits = MM_VM_RESERVED / CONFIG_MMU_PAGE_SIZE;
		(void)sys_bitarray_set_region(&virt_region_bitmap,
					      num_bits, 0);
	}

	/* Mark all bits up to MM_VM_FREE_START as allocated */
	num_bits = POINTER_TO_UINT(MM_VM_FREE_START)
		   - POINTER_TO_UINT(MM_VIRT_RAM_START);
	offset = virt_to_bitmap_offset(MM_VIRT_RAM_START, num_bits);
...
static void *virt_region_alloc(size_t size, size_t align)
{
	...
	if (alloc_size > size) {
		 ...
		 * | Unused       |
		 * +==============+ <- offset from MM_VIRT_RAM_END == dest_addr + alloc_size
		 * | ...          |
		 * +--------------+
		 * | Mapping      |
		 * +--------------+
...
#define PF_ASSERT(pf, expr, fmt, ...) \
	__ASSERT(expr, "page frame 0x%lx: " fmt, mm_vm_page_frame_to_phys(pf), \
		 ##__VA_ARGS__)

/* Get an unused page frame. don't care which one, or NULL if there are none */
static struct mm_vm_page_frame *free_page_frame_list_get(void)
{
	sys_sfnode_t *node;
	struct mm_vm_page_frame *pf = NULL;

	node = sys_sflist_get(&free_page_frame_list);
	if (node != NULL) {
		z_free_page_count--;
		pf = CONTAINER_OF(node, struct mm_vm_page_frame, node);
		PF_ASSERT(pf, mm_vm_page_frame_is_free(pf),
...
static void free_page_frame_list_put(struct mm_vm_page_frame *pf)
{
	PF_ASSERT(pf, mm_vm_page_frame_is_available(pf),
		 "unavailable page put on free list");

	sys_sfnode_init(&pf->node, MM_VM_PAGE_FRAME_FREE);
...
static void page_frame_free_locked(struct mm_vm_page_frame *pf)
{
	pf->va_and_flags = 0;
 ...
 */
static void frame_mapped_set(struct mm_vm_page_frame *pf, void *addr)
{
	PF_ASSERT(pf, !mm_vm_page_frame_is_free(pf),
		  "attempted to map a page frame on the free list");
	PF_ASSERT(pf, !mm_vm_page_frame_is_reserved(pf),
	 ...
	 */
	PF_ASSERT(pf, !mm_vm_page_frame_is_mapped(pf) || mm_vm_page_frame_is_pinned(pf),
		 "non-pinned and already mapped to %p",
		 mm_vm_page_frame_to_virt(pf));

	uintptr_t flags_mask = CONFIG_MMU_PAGE_SIZE - 1;
	uintptr_t va = (uintptr_t)addr & ~flags_mask;

	pf->va_and_flags &= flags_mask;
	pf->va_and_flags |= va | MM_VM_PAGE_FRAME_MAPPED;
}
...
{
	uintptr_t paddr;
	struct mm_vm_page_frame *pf;
	int ret = -EFAULT;

	MM_VM_PAGE_FRAME_FOREACH(paddr, pf) {
		if (mm_vm_page_frame_is_mapped(pf)) {
			if (virt == mm_vm_page_frame_to_virt(pf)) {
				ret = 0;
				if (phys != NULL) {
					*phys = mm_vm_page_frame_to_phys(pf);
				}
				break;
			}
		}
...
#ifdef CONFIG_DEMAND_PAGING
static int page_frame_prepare_locked(struct mm_vm_page_frame *pf, bool *dirty_ptr,
...
static int map_anon_page(void *addr, uint32_t flags)
{
	...
	if (lock) {
		mm_vm_page_frame_set(pf, MM_VM_PAGE_FRAME_PINNED);
	}
...
void *mm_memmap_phys_guard_map(uintptr_t phys, size_t size, uint32_t flags, bool is_anon)
{
	uint8_t *dst;
	size_t total_size;
	int ret;
...
void mm_memmap_phys_guard_unmap(void *addr, size_t size, bool is_anon)
{
	uintptr_t phys;
	uint8_t *pos;
	struct mm_vm_page_frame *pf;
	...
	mm_vm_assert_virtual_region(pos, size + (CONFIG_MMU_PAGE_SIZE * 2));
	...
	if (is_anon) {
		...
		VIRT_FOREACH(addr, size, pos) {
			...
			pf = mm_vm_phys_to_page_frame(phys);

			__ASSERT(mm_vm_page_frame_is_mapped(pf),
 ...
 */
void mm_memmap_phys_bare_map(uint8_t **virt_ptr, uintptr_t phys, size_t size, uint32_t flags)
{
	...
	if (IS_ENABLED(CONFIG_KERNEL_DIRECT_MAP) &&
		...
		if (IN_RANGE(aligned_phys,
		    ...
		    IN_RANGE(aligned_phys + aligned_size - 1,
			      (uintptr_t)MM_VIRT_RAM_START,
			      (uintptr_t)(MM_VIRT_RAM_END - 1))) {
			uint8_t *adjusted_start = MAX(dest_addr, MM_VIRT_RAM_START);
...
void mm_memmap_phys_bare_unmap(uint8_t *virt, size_t size)
{
				       ...
				       bool pin)
{
	...
	{
		pf = mm_vm_phys_to_page_frame(MM_MEMMAP_BOOT_VIRT_TO_PHYS(addr));
		frame_mapped_set(pf, addr);

		if (pin) {
			mm_vm_page_frame_set(pf, MM_VM_PAGE_FRAME_PINNED);
		} else {
			mm_vm_page_frame_clear(pf, MM_VM_PAGE_FRAME_PINNED);
		}
	}
}
...
void z_mem_manage_init(void)
{
	 ...
	 * code will mark MM_VM_PAGE_FRAME_RESERVED in their flags
	 */
	 ...
	 */
	VIRT_FOREACH(MM_KERNEL_VIRT_START, MM_KERNEL_VIRT_SIZE, addr)
	{
		pf = mm_vm_phys_to_page_frame(MM_MEMMAP_BOOT_VIRT_TO_PHYS(addr));
		frame_mapped_set(pf, addr);
	 ...
	 */
	MM_VM_PAGE_FRAME_FOREACH(phys, pf) {
		if (mm_vm_page_frame_is_available(pf)) {
			free_page_frame_list_put(pf);
		}
	}
...
static void virt_region_foreach(void *addr, size_t size,
				void (*func)(void *))
{
	mm_vm_assert_virtual_region(addr, size);
				     ...
				     bool page_fault, uintptr_t *location_ptr)
{
	...
	phys = mm_vm_page_frame_to_phys(pf);
	__ASSERT(!mm_vm_page_frame_is_pinned(pf), "page frame 0x%lx is pinned",
		 phys);
	 ...
	 */
	if (mm_vm_page_frame_is_mapped(pf)) {
		dirty = dirty || !mm_vm_page_frame_is_backed(pf);
	}
	...
	if (mm_vm_page_frame_is_mapped(pf)) {
		...
		arch_mem_page_out(mm_vm_page_frame_to_virt(pf), *location_ptr);
	} else {
	...
	/* Mark as busy so that mm_vm_page_frame_is_evictable() returns false */
	__ASSERT(!mm_vm_page_frame_is_busy(pf), "page frame 0x%lx is already busy",
		 phys);
	mm_vm_page_frame_set(pf, MM_VM_PAGE_FRAME_BUSY);
...
static int do_mem_evict(void *addr)
{
	...
	pf = mm_vm_phys_to_page_frame(phys);
	__ASSERT(mm_vm_page_frame_to_virt(pf) == addr, "page frame address mismatch");
	ret = page_frame_prepare_locked(pf, &dirty, false, &location);
	if (ret != 0) {
		goto out;
	}
...
int mm_vm_page_frame_evict(uintptr_t phys)
{
	...
	flags = arch_page_info_get(mm_vm_page_frame_to_virt(pf), NULL, false);
...
static inline struct mm_vm_page_frame *do_eviction_select(bool *dirty)
{
	struct mm_vm_page_frame *pf;
...
#ifdef CONFIG_DEMAND_PAGING_ALLOW_IRQ
	key = irq_lock();
	mm_vm_page_frame_clear(pf, MM_VM_PAGE_FRAME_BUSY);
#endif /* CONFIG_DEMAND_PAGING_ALLOW_IRQ */
	mm_vm_page_frame_clear(pf, MM_VM_PAGE_FRAME_MAPPED);
	frame_mapped_set(pf, addr);
	if (pin) {
		mm_vm_page_frame_set(pf, MM_VM_PAGE_FRAME_PINNED);
	}

	arch_mem_page_in(addr, mm_vm_page_frame_to_phys(pf));
