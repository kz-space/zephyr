...
static struct k_sem sem_ba_discovered;
...
static void init(size_t acceptor_cnt)
{
	...
	if (err != 0) {
		FAIL("Failed to register broadcast sink callbacks (err %d)\n");
		return;
	}
	...
	k_sem_init(&sem_ba_discovered, 0, acceptor_cnt);
...
static void discover_ba(size_t acceptor_cnt)
{
	k_sem_reset(&sem_ba_discovered);
	...
	for (size_t i = 0U; i < acceptor_cnt; i++) {
		const int err = k_sem_take(&sem_ba_discovered, SEM_TIMEOUT);

		if (err != 0) {
			FAIL("Failed to take sem_ba_discovered for %p: %d",
			     (void *)connected_conns[i], err);
		}
	}
}

static void sync_to_broadcaster(void)
{
	int err;
...
{
	struct bt_cap_commander_broadcast_reception_start_param reception_start_param;
	...
	for (size_t i = 0; i < accepter_count; i++) {
		...
		reception_start_param.param[i].num_subgroups = 0;
		err = bt_bap_base_foreach_subgroup((const struct bt_bap_base *)received_base,
						   add_pa_sync_base_subgroup_cb, &param[i]);
		if (err != 0) {
	...
	backchannel_sync_wait(2); /* wait for the acceptor to receive data */

	backchannel_sync_wait(2); /* wait for the acceptor to receive a metadata update */
}
...
static void test_broadcast_reception_stop(size_t accepter_count)
{
	...
	backchannel_sync_wait(2); /* wait for the acceptor to stop reception */
}
