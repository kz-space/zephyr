...
/* mm_memmap_phys_bare_map() doesn't have alignment requirements, any oddly-sized buffer
 ...
 */
ZTEST(mem_map, test_mm_memmap_phys_bare_map_rw)
{
	...
	mm_memmap_phys_bare_map(&mapped_rw, mm_memmap_phys_addr(buf),
				BUF_SIZE, BASE_FLAGS | K_MEM_PERM_RW);

	/* Map again this time only allowing reads */
	mm_memmap_phys_bare_map(&mapped_ro, mm_memmap_phys_addr(buf),
				BUF_SIZE, BASE_FLAGS);
 ...
 * @ingroup kernel_memprotect_tests
 */
ZTEST(mem_map, test_mm_memmap_phys_bare_map_exec)
{
#ifndef SKIP_EXECUTE_TESTS
	...
	mm_memmap_phys_bare_map(&mapped_exec, mm_memmap_phys_addr(__test_mem_map_start),
	...
	mm_memmap_phys_bare_map(&mapped_ro, mm_memmap_phys_addr(__test_mem_map_start),
				(uintptr_t)(__test_mem_map_end - __test_mem_map_start),
				BASE_FLAGS);
 ...
 * @ingroup kernel_memprotect_tests
 */
ZTEST(mem_map, test_mm_memmap_phys_bare_map_side_effect)
{
	uint8_t *mapped;

	expect_fault = false;

	/* mm_memmap_phys_bare_map() is supposed to always create fresh mappings.
	 * Show that by mapping test_page to an RO region, we can still
	 * modify test_page.
	 */
	mm_memmap_phys_bare_map(&mapped, mm_memmap_phys_addr(test_page),
...
/**
 * Test that mm_memmap_phys_bare_unmap() unmaps the memory and it is no longer
 * accessible afterwards.
 *
 * @ingroup kernel_memprotect_tests
 */
ZTEST(mem_map, test_mm_memmap_phys_bare_unmap)
{
	uint8_t *mapped;
	...
	/* Unmap the memory */
	mm_memmap_phys_bare_unmap(mapped, sizeof(test_page));
...
/**
 * Show that mm_memmap_phys_bare_unmap() can reclaim the virtual region correctly.
 *
 * @ingroup kernel_memprotect_tests
 */
ZTEST(mem_map, test_mm_memmap_phys_bare_map_unmap_reclaim_addr)
{
	uint8_t *mapped, *mapped_old;
	uint8_t *buf = test_page + BUF_OFFSET;

	/* Map the buffer the first time. */
	mm_memmap_phys_bare_map(&mapped, mm_memmap_phys_addr(buf),
				BUF_SIZE, BASE_FLAGS);
	 ...
	 * so that the next time mm_memmap_phys_bare_map() is called with
	 * the same arguments, it will return the same address.
	 */
	mm_memmap_phys_bare_unmap(mapped, BUF_SIZE);
...
ZTEST(mem_map_api, test_k_mem_map_exhaustion)
{
	 ...
	 *    MM_VM_FREE_START to end address.
	...
	cnt -= POINTER_TO_UINT(MM_VM_FREE_START);
...
ZTEST(mem_map_api, test_k_mem_map_user)
{
	...
	mm_memmap_phys_bare_map(&mapped, mm_memmap_phys_addr(test_page), sizeof(test_page),
