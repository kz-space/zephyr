			 ...
			 int line)
{
	...
	zassert_equal(exp_m_idx, ids[packet.log->hdr.master], "%d: Unexpected master:%d (exp:%d)",
		      line, packet.log->hdr.master, exp_m_idx);
			      ...
			      uint32_t *exp_data, int line)
{
	...
	zassert_equal(exp_m_idx, ids[packet.trace_point->master],
		      "%d: Unexpected master:%d (exp:%d)", line, packet.trace_point->master,
		      exp_m_idx);
	if (exp_data) {
...
{
	if (m_id) {
		log_frontend_stmesp_demux_master(*m_id);
	}
...
static int write_hw_event(uint8_t evt, uint64_t ts)
{
	...
	log_frontend_stmesp_demux_master(M_HW);
