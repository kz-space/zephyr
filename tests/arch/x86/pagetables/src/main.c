...
ZTEST(x86_pagetables, test_ram_perms)
{
	 ...
	 */
	for (pos = (uint8_t *)MM_PHYS_RAM_START;
	     pos < (uint8_t *)MM_PHYS_RAM_END;
	     pos += CONFIG_MMU_PAGE_SIZE) {
		if (pos == NULL) {
			continue;
		}
