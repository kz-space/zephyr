 ...
 * MM_VM_PAGE_FRAME_BACKED is never set, even if the data page was paged out before
 ...
 * a paged-in data page in a custom field of its associated mm_vm_page_frame, and
 * set the MM_VM_PAGE_FRAME_BACKED bit. Invocations of
 * k_mem_paging_backing_store_location_get() will have logic to return
 * the previous clean page location instead of allocating
 * a new one if mm_vm_PAGE_FRAME_BACKED is set.
 *
 ...
 * as the backing store fills up, and clear the MM_VM_PAGE_FRAME_BACKED bit
 * appropriately.
 *
 * All of this logic is local to the backing store implementation; from the
 * core kernel's perspective the only change is that MM_VM_PAGE_FRAME_BACKED
...
{
	(void)memcpy(location_to_slab(location), MM_VM_SCRATCH_PAGE,
		     CONFIG_MMU_PAGE_SIZE);
}

void k_mem_paging_backing_store_page_in(uintptr_t location)
{
	(void)memcpy(MM_VM_SCRATCH_PAGE, location_to_slab(location),
		     CONFIG_MMU_PAGE_SIZE);
}
