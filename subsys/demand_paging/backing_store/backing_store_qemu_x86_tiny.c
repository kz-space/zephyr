					    ...
					    bool page_fault)
{
	...
	*location = POINTER_TO_UINT(mm_vm_page_frame_to_virt(pf));
...
{
	(void)memcpy(location_to_flash(location), MM_VM_SCRATCH_PAGE,
		     CONFIG_MMU_PAGE_SIZE);
}

void k_mem_paging_backing_store_page_in(uintptr_t location)
{
	(void)memcpy(MM_VM_SCRATCH_PAGE, location_to_flash(location),
		     CONFIG_MMU_PAGE_SIZE);
}
