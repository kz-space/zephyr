					 ...
					 struct ptp_tlv_mgmt *req_mgmt)
{
	...
	case PTP_MGMT_DEFAULT_DATA_SET:
		...
		tlv_dds->flags = 0x1 | (dds->slave_only << 1);
	...
	case PTP_MGMT_SLAVE_ONLY:
		length = sizeof(dds->slave_only);
		*mgmt->data = dds->slave_only;
		break;
...
int port_state_update(struct ptp_port *port, enum ptp_port_event event, bool master_diff)
{
...
void ptp_port_event_handle(struct ptp_port *port, enum ptp_port_event event, bool master_diff)
{
	...
	if (!port_state_update(port, event, master_diff)) {
