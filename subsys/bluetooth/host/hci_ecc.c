...
#if defined(CONFIG_BT_USE_PSA_API)
static void psa_key_set_attributes(psa_key_attributes_t *attr)
{
...
static uint8_t generate_keys(void)
{
	...
	if (psa_export_public_key(key_id, ecc.public_key_be, BT_PUB_KEY_LEN,
				&tmp_len) != PSA_SUCCESS) {
		LOG_ERR("Failed to create ECC public key");
		return BT_HCI_ERR_UNSPECIFIED;
	}

	if (psa_export_key(key_id, ecc.private_key_be, BT_PUB_KEY_LEN,
			&tmp_len) != PSA_SUCCESS) {
		LOG_ERR("Failed to create ECC private key");
		return BT_HCI_ERR_UNSPECIFIED;
	}

	if (psa_destroy_key(key_id) != PSA_SUCCESS) {
		LOG_ERR("Failed to destroy ECC key");
		return BT_HCI_ERR_UNSPECIFIED;
	}
...
static void emulate_le_generate_dhkey(void)
{
	...
	if (psa_raw_key_agreement(PSA_ALG_ECDH, key_id, ecc.public_key_be,
			BT_PUB_KEY_LEN, ecc.dhkey_be, BT_DH_KEY_LEN, &tmp_len) != PSA_SUCCESS) {
		ret = -EIO;
		goto exit;
	}
