...
{
	return mm_memmap_virt_addr(get_entry_phys(entry, level));
}
...
void z_x86_tlb_ipi(const void *arg)
{
	...
	__ASSERT(ptables_phys == mm_memmap_phys_addr(&z_x86_kernel_ptables), "");
#else
	 ...
	 */
	ptables_phys = mm_memmap_phys_addr(z_x86_thread_page_tables_get(_current));
#endif
	/*
		...
		if ((entry & MMU_P) != 0U) {
			if (is_leaf(level, entry)) {
				if (phys == virt) {
					/* Identity mappings */
					COLOR(YELLOW);
				} else if (phys + MM_MEMMAP_VIRT_OFFSET == virt) {
					  ...
					  int level)
{
	...
	static const uintptr_t shared_phys_addr =
		MM_MEMMAP_PHYS_ADDR(POINTER_TO_UINT(&z_shared_kernel_page_start));
...
static int copy_page_table(pentry_t *dst, pentry_t *src, int level)
{
	if (level == PTE_LEVEL) {
		...
		for (int i = 0; i < get_num_entries(level); i++) {
			 ...
			 */
			dst[i] = ((pentry_t)mm_memmap_phys_addr(child_dst) |
				  INT_FLAGS);
...
int arch_mem_domain_thread_add(struct k_thread *thread)
{
	 ...
	 */
	if (is_migration) {
		old_ptables = mm_memmap_virt_addr(thread->arch.ptables);
		set_stack_perms(thread, domain->arch.ptables);
	}

	thread->arch.ptables = mm_memmap_phys_addr(domain->arch.ptables);
...
{
	page_map_set(z_x86_page_tables_get(), MM_VM_SCRATCH_PAGE,
