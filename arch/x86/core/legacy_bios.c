...
static uintptr_t bios_search_rsdp_buff(uintptr_t search_phy_add, uint32_t search_length)
{
	...
	mm_memmap_phys_bare_map((uint8_t **)&search_buff, search_phy_add, search_length, 0);
	if (!search_buff) {
		return 0;
	}
	...
	for (int i = 0; i < search_length / 8u; i++) {
		if (search_buff[i] == RSDP_SIGNATURE) {
			mm_memmap_phys_bare_unmap((uint8_t *)search_buff, search_length);
...
void *bios_acpi_rsdp_get(void)
{
	...
	mm_memmap_phys_bare_map(&zero_page_base, 0, DATA_SIZE_K(4u), 0);
	bios_ext_data = EBDA_ADD + zero_page_base;
	search_phy_add = (uintptr_t)((*(uint16_t *)bios_ext_data) << 4u);
	mm_memmap_phys_bare_unmap(zero_page_base, DATA_SIZE_K(4u));
