 ...
 * @brief Stepper Motor Controller shall relatively move the motor with given steps
 */
...
/**
 * @typedef stepper_set_stall_detection_threshold_value_t
 * @brief Set stepper motor stall detection threshold value
 */
typedef int32_t (*stepper_set_stall_detection_threshold_value_t)(const struct device *dev,
								 const int32_t value);

/**
 * @typedef stepper_enable_stall_detection_t
 * @brief enable/disable the stall detection int the stepper motor controller
 */
typedef int32_t (*stepper_enable_stall_detection_t)(const struct device *dev,
						    const bool enable_stall_detection);

/**
 * @typedef stepper_detect_load_t
 * @brief detect the load on a stepper motor
 */
typedef int32_t (*stepper_detect_load_t)(const struct device *dev, int32_t *load);
...
__subsystem struct stepper_api {
	...
	stepper_set_stall_detection_threshold_value_t set_stall_detection_threshold;
	stepper_enable_stall_detection_t enable_stall_detection;
	stepper_detect_load_t detect_load;
};
 ...
 * stopped using some other command.
 * @param dev pointer to the stepper motor controller instance
 * @param value The velocity to set
 *		> 0: Enable constant velocity mode with the given velocity in positive direction
 ...
 * @param dev pointer to the stepper motor controller instance
 * @param value threshold value to be set
 * @retval -ENOSYS If not implemented by device driver
 * @retval       0 Success
 */
__syscall int32_t stepper_set_stall_detection_threshold(const struct device *dev,
							const int32_t value);

static inline int32_t z_impl_stepper_set_stall_detection_threshold(const struct device *dev,
								   const int32_t value)
{
	const struct stepper_api *api = (const struct stepper_api *)dev->api;

	if (api->set_stall_detection_threshold == NULL) {
		return -ENOSYS;
	}
	return api->set_stall_detection_threshold(dev, value);
}

/**
 * @brief Enable the stall detection of the stepper motor
 * @param dev pointer to the stepper motor controller instance
 * @param enable_stall_detection true: enable stall detection, false: disable stall detection
 * @retval -ENOSYS If not implemented by device driver
 * @retval       0 Success
 */
__syscall int32_t stepper_enable_stall_detection(const struct device *dev,
						 const bool enable_stall_detection);

static inline int32_t z_impl_stepper_enable_stall_detection(const struct device *dev,
							    const bool enable_stall_detection)
{
	const struct stepper_api *api = (const struct stepper_api *)dev->api;

	if (api->enable_stall_detection == NULL) {
		return -ENOSYS;
	}
	return api->enable_stall_detection(dev, enable_stall_detection);
}

/**
 * @brief Detect the load on a stepper motor
 * @param dev pointer to the stepper motor controller instance
 * @param load load value as detected by the motor controller
 * @retval -ENOSYS If not implemented by device driver
 * @retval       0 Success
 */
__syscall int32_t stepper_detect_load(const struct device *dev, int32_t *load);

static inline int32_t z_impl_stepper_detect_load(const struct device *dev, int32_t *load)
{
	const struct stepper_api *api = (const struct stepper_api *)dev->api;

	if (api->stepper_detect_load == NULL) {
		return -ENOSYS;
	}
	return api->detect_load(dev, load);
}
