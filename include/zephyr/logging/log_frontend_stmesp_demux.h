...
/** @brief Bits used to store master index. */
#define LOG_FRONTEND_STMESP_DEMUX_MASTER_BITS 3
...
/** @brief Maximum number of supported masters. */
#define LOG_FRONTEND_STMESP_DEMUX_MASTER_MAX BIT(LOG_FRONTEND_STMESP_DEMUX_MASTER_BITS)
...
struct log_frontend_stmesp_demux_log_header {
	/** Master index. */
	uint32_t master : LOG_FRONTEND_STMESP_DEMUX_MASTER_BITS;
...
struct log_frontend_stmesp_demux_config {
	/** Array with expected master ID's. */
	const uint16_t *m_ids;

	/** Array length. Must be not bigger than @ref LOG_FRONTEND_STMESP_DEMUX_MASTER_MAX. */
	uint32_t m_ids_cnt;
};
...
/** @brief Indicate master opcode in the STPv2 stream.
 *
 * @param id	Master ID.
 */
void log_frontend_stmesp_demux_master(uint16_t id);
