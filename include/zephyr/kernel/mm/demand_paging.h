 ...
 */
struct mm_vm_page_frame *k_mem_paging_eviction_select(bool *dirty);
 ...
 * page frame has its MM_VM_PAGE_FRAME_BACKED bit set, it is expected to return
 ...
 * will have the MM_VM_PAGE_FRAME_BACKED bit cleared (as it is no longer cached).
 *
 * mm_vm_page_frame_to_virt(pf) will indicate the virtual address the page is
 ...
 */
int k_mem_paging_backing_store_location_get(struct mm_vm_page_frame *pf,
					    uintptr_t *location,
					    bool page_fault);
...
/**
 * Copy a data page from MM_VM_SCRATCH_PAGE to the specified location
 *
 * Immediately before this is called, MM_VM_SCRATCH_PAGE will be mapped read-write
...
/**
 * Copy a data page from the provided location to MM_VM_SCRATCH_PAGE.
 *
 * Immediately before this is called, MM_VM_SCRATCH_PAGE will be mapped read-write
 ...
 * have been* re-locked, making it safe to access the mm_vm_page_frame data.
 ...
 * time to set the MM_VM_PAGE_FRAME_BACKED bit. The kernel only skips paging
 * out clean data pages if they are noted as clean in the page tables and the
 * MM_VM_PAGE_FRAME_BACKED bit is set in their associated page frame.
 *
 * @param pf Page frame that was loaded in
 * @param location Location of where the loaded data page was retrieved
 */
void k_mem_paging_backing_store_page_finalize(struct mm_vm_page_frame *pf,
					      uintptr_t location);
 ...
 *   at boot time, MM_VM_PAGE_FRAME_BACKED should be appropriately set for their
