...
snippets from this file are used as examples. You cannot use this source file
and Kconfig options directly in your application.
...
Your USB device can use manufacturer, product, and serial number string
descriptors. The application should use the appropriate
:c:macro:`USBD_DESC_MANUFACTURER_DEFINE`, :c:macro:`USBD_DESC_PRODUCT_DEFINE`,
and :c:macro:`USBD_DESC_SERIAL_NUMBER_DEFINE` macros. Optional string
descriptors also require a single instantiation of the language descriptor
using the :c:macro:`USBD_DESC_LANG_DEFINE` macro.
...
USB device requires at least one configuration. If the USB controller assigned
to the device context supports high speed, it needs at least one configuration
instance per speed. The application should use
:c:macro:`USBD_CONFIGURATION_DEFINE` to instantiate a configuration. Later, USB
device functions are assigned to a configuration.
...
Any string descriptor must be added to the device context at runtime before
...
:c:func:`usbd_add_configuration`. See :c:enumerator:`USBD_SPEED_FS` and
:c:enumerator:`USBD_SPEED_HS`. For each of the spedific speed configuration
number is assigned based on the order of addition to the context, means the
first full speed configuration will get number one, so called high speed, and
further accordingly.
...
can have multiple configuration with different set of function, event at
different speed. A function or class can be registered on a USB device before
it is initialized using :c:func:`usbd_register_class`. The specific
