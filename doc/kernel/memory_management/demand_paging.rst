...
Page Frame
  ...
  addresses. For every page frame, a ``struct mm_vm_page_frame`` is instantiated to
  store metadata. Flags for each page frame:

  * ``MM_VM_PAGE_FRAME_FREE`` indicates a page frame is unused and on the list of
    free page frames. When this flag is set, none of the other flags are
    meaningful and they must not be modified.

  * ``MM_VM_PAGE_FRAME_PINNED`` indicates a page frame is pinned in memory
    and should never be paged out.

  * ``MM_VM_PAGE_FRAME_RESERVED`` indicates a physical page reserved by hardware
    and should not be used at all.

  * ``MM_VM_PAGE_FRAME_MAPPED`` is set when a physical page is mapped to
    virtual memory address.

  * ``MM_VM_PAGE_FRAME_BUSY`` indicates a page frame is currently involved in
    a page-in/out operation.

  * ``MM_VM_PAGE_FRAME_BACKED`` indicates a page frame has a clean copy
    in the backing store.

MM_VM_SCRATCH_PAGE
  The virtual address of a special page provided to the backing store to:
  * Copy a data page from ``MM_VM_SCRATCH_PAGE`` to the specified location; or,
...
* :c:func:`k_mem_paging_backing_store_page_in()` copies a data page
  ...
  ``location`` token to the page pointed by ``MM_VM_SCRATCH_PAGE``.

* :c:func:`k_mem_paging_backing_store_page_out()` copies a data page
  from ``MM_VM_SCRATCH_PAGE`` to the backing store location associated
