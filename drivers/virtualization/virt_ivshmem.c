...
static bool ivshmem_configure(const struct device *dev)
{
	...
	if (data->ivshmem_v2) {
		...
		mm_memmap_phys_bare_map((uint8_t **)&data->state_table_shmem,
		...
		if (data->rw_section_size > 0) {
			mm_memmap_phys_bare_map((uint8_t **)&data->rw_section_shmem,
		...
		for (uint32_t i = 0; i < data->max_peers; i++) {
			...
			mm_memmap_phys_bare_map((uint8_t **)&data->output_section_shmem[i],
...
#endif /* CONFIG_IVSHMEM_V2 */
	{
		...
		mm_memmap_phys_bare_map((uint8_t **)&data->shmem,
