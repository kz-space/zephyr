...
static void vevif_event_rx_isr(const void *device)
{
	...
	for (uint8_t id = EVENTS_IDX_MIN; id < EVENTS_IDX_MAX + 1U; id++) {
		nrf_vpr_event_t event = nrf_vpr_triggered_event_get(id);

		if (nrf_vpr_event_check(config->vpr, event)) {
			nrf_vpr_event_clear(config->vpr, event);
...
static int vevif_event_rx_set_enabled(const struct device *dev, uint32_t id, bool enable)
{
	...
	if (enable) {
		...
		nrf_vpr_int_enable(config->vpr, BIT(id));
	} else {
		if ((cbs->enabled_mask & BIT(id)) == 0U) {
			return -EALREADY;
		}
		...
		cbs->enabled_mask &= ~BIT(id);
		nrf_vpr_int_disable(config->vpr, BIT(id));
	}
...
#define VEVIF_EVENT_LOCAL_DEFINE(inst)                                                             \
...
DT_INST_FOREACH_STATUS_OKAY(VEVIF_EVENT_LOCAL_DEFINE)
