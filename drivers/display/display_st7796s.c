...
struct st7796s_config {
	...
	uint8_t bus_width; /* Data bus width in 8080 mode */
};
...
static int st7796s_get_pixelfmt(const struct device *dev)
{
	 ...
	 * and vice versa.
	 */
	if ((config->dbi_config.mode == MIPI_DBI_MODE_8080_BUS) &&
	    (config->bus_width == 8)) {
	...
	static const struct st7796s_config st7796s_config_##n = {		\
		...
		.dbi_config = {							\
			.config = MIPI_DBI_SPI_CONFIG_DT(			\
						...
						SPI_WORD_SET(8),		\
						0),				\
			.mode = DT_INST_PROP_OR(n, mipi_mode, 0),		\
		},								\
		...
		.bus_width = DT_PROP_OR(DT_INST_PARENT(n), data_bus_width, 0),	\
	};									\
										\
