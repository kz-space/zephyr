...
static int rcar_mmc_dma_rx_tx_data(const struct device *dev, struct sdhc_data *data, bool is_read)
{
	...
	dma_addr = mm_memmap_phys_addr(data->data);
...
#ifdef CONFIG_RCAR_MMC_DMA_SUPPORT
	if (!(mm_memmap_phys_addr(data->data) >> 32)) {
...
exit_unmap:
	...
	mm_memmap_phys_bare_unmap((uint8_t *)DEVICE_MMIO_GET(dev), DEVICE_MMIO_ROM_PTR(dev)->size);
#endif
	return ret;
}
