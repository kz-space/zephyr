...
static bool get_extend_enable_bit(const uint8_t ep_idx)
{
	...
	if (((ep_idx - 4) / 3 == 0)) {
		enable = !!(epn_ext_ctrl1->fields.epn0_enable_bit);
	} else if (((ep_idx - 4) / 3 == 1)) {
		enable = !!(epn_ext_ctrl1->fields.epn3_enable_bit);
	} else if (((ep_idx - 4) / 3 == 2)) {
		enable = !!(epn_ext_ctrl1->fields.epn6_enable_bit);
	} else {
		enable = !!(epn_ext_ctrl1->fields.epn9_enable_bit);
	}
	return enable;
}
...
static bool it82xx2_usb_fake_token(const uint8_t ep_idx, uint8_t *token_type)
{
	...
	if (IT8XXX2_IS_EXTEND_ENDPOINT(ep_idx)) {
		...
		enable_bit = !!(ep_regs[ep_idx].ep_ctrl.fields.enable_bit);
