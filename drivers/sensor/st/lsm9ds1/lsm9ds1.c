...
static int lsm9ds1_accel_config(const struct device *dev,
...
static int lsm9ds1_gyro_config(const struct device *dev,
			       enum sensor_channel chan,
			       enum sensor_attribute attr,
			       const struct sensor_value *val)
{
...
static inline int lsm9ds1_accel_get_channel(enum sensor_channel chan,
...
static int lsm9ds1_accel_channel_get(enum sensor_channel chan,
				     struct sensor_value *val,
				     struct lsm9ds1_data *data)
{
...
static inline void lsm9ds1_gyro_convert(struct sensor_value *val, int raw_val,
					uint32_t sensitivity)
{
	int64_t dval;
...
static inline int lsm9ds1_gyro_get_channel(enum sensor_channel chan,
					   struct sensor_value *val,
					   struct lsm9ds1_data *data,
					   uint32_t sensitivity)
{
...
static int lsm9ds1_gyro_channel_get(enum sensor_channel chan,
				    struct sensor_value *val,
				    struct lsm9ds1_data *data)
{
...
static int lsm9ds1_init(const struct device *dev)
{
	...
	if (chip_id != LSM9DS1_IMU_ID) {
		LOG_DBG("Invalid ID : got %x", chip_id);
		return -EIO;
	}
...
#define LSM9DS1_DEFINE(inst)								\
	static struct lsm9ds1_data lsm9ds1_data_##inst = {.acc_gain = 0,};		\
											\
	static struct lsm9ds1_config lsm9ds1_config_##inst = LSM9DS1_CONFIG_I2C(inst);	\
											\
											\
	SENSOR_DEVICE_DT_INST_DEFINE(inst, lsm9ds1_init, NULL,				\
				&lsm9ds1_data_##inst, &lsm9ds1_config_##inst,		\
