 ...
 * @param buffer_out Bus address of the DMA output buffer previously prepared with @ref
 * dmm_dma_buffer_out_prepare().
 *
 * @retval 0 If succeeded.
 ...
 *                    from the given device.
 * @param user_length Length of the buffer to be filled with data from the given device..
 * @param buffer_in Bus address of the DMA input buffer previously prepared with @ref
 * dmm_dma_buffer_in_prepare().
 *
...
static ALWAYS_INLINE int dmm_buffer_out_prepare(void *region, void *user_buffer, size_t user_length,
						void **buffer_out)
{
	ARG_UNUSED(region);
